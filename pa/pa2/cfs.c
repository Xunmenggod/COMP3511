...
int sum_of_weights = 0; // record the sum of weights of all processes
int step_count = 0;
...
void init_cfs_process()
{
    ...
    for (int i = 0; i < num_process; i++)
    {
     ...
     for (int i = 0; i < num_process; i++)
    {
        process[i].time_slice = (int)((double)process[i].weight * sched_latency / sum_of_weights);
        process[i].time_slice = (process[i].time_slice < min_granularity) ? min_granularity : process[i].time_slice;
    }
    printf(template_cfs_algorithm);
    printf(template_step_i, step_count++);
    print_cfs_process();
}
...
void run_cfs_scheduling()
{
    ...
    // the boolean array to record which process has been finished or not
    int isFinihsed[MAX_PROCESS] = {0}; 
    int smallest_vruntime_process = -1;
    double smallest_vruntime = 99999.0;
    while(finish_process_count < num_process)
    {   
        int run_time = 0;
        smallest_vruntime = 99999.0;
        // this loop if for selecting the smallest vruntime process
        for (int i = 0; i < num_process; i++)
        {
            if (!isFinihsed[i])
            {
               if (smallest_vruntime > process[i].vruntime)
               {
                    smallest_vruntime = process[i].vruntime;
                    smallest_vruntime_process = i;
               }
            }
        }

        // run the selected process and update process table
        if (process[smallest_vruntime_process].remain_time > process[smallest_vruntime_process].time_slice)
            run_time = process[smallest_vruntime_process].time_slice;
        else
            run_time = process[smallest_vruntime_process].remain_time;
        process[smallest_vruntime_process].remain_time -= run_time;
        process[smallest_vruntime_process].vruntime += (double)DEFAULT_WEIGHT / process[smallest_vruntime_process].weight * run_time;
        printf(template_step_i, step_count++);
        print_cfs_process();

        // update chart
        if (num_chart_item > 0 && smallest_vruntime_process == chart[num_chart_item - 1].pid)
            chart[num_chart_item - 1].duration += run_time;
        else{
            chart[num_chart_item].pid = smallest_vruntime_process;
            chart[num_chart_item++].duration = run_time;
        }

        // remove a finished process
        if (process[smallest_vruntime_process].remain_time == 0)
        {
            finish_process_count++;
            isFinihsed[smallest_vruntime_process] = 1;
            sum_of_weights -= process[smallest_vruntime_process].weight;
        }
    }
}
